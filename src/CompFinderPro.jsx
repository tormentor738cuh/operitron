...
const toolsEs = [
  ...
  ["loan", "Calculadora de Pr?stamo de Inversi?n", "Modela apalancamiento, puntos, pago y DSCR.", Calculator, "DSCR"],
  ["todo", "Lista de Tareas", "Rastrea próximos pasos de inversionistas, prestamistas y contratistas.", ListChecks, "Live"],
  ["punch", "Lista de Pendientes", "Registra detalles de inspecci?n final antes del cierre.", ClipboardCheck, "Oficios"],
      ...
      <section className="grid min-w-0 items-start gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(320px,390px)]">
        <GlassPanel className="self-start">
          ...
          {projects.length ? <div className="grid gap-5 lg:grid-cols-2">
            ...
            <p className="mt-3 font-black text-white">{language === "es" ? "A?n no hay proyectos guardados" : "No saved projects yet"}</p>
...
function PublicHome({ t, go }) {
  ...
  const publicCards = isEs
    ? [
      ["Análisis de Deals", "Analiza flips, rentas, BRRR y oportunidades de construcción con f?rmulas para inversionistas.", LineChart],
...
function LandingStats({ isEs }) {
  const stats = isEs
    ? [["8", "Herramientas de Proyecto"], ["14", "Fases de Construcción"], ["2", "Planes Transparentes"], ["3 D?as", "Prueba de Acceso Completo"]]
...
function Testimonials({ isEs }) {
  const quotes = isEs
    ? [
      { quote: "ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“Operitron nos ayud? a revisar más deals sin perder control de los supuestos.ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â", initial: "C", name: "Camila S.", role: "Desarrolladora de Vivienda", location: "Phoenix, AZ", color: "from-cyan-400 to-blue-600" },
      { quote: "ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“La combinaci?n de underwriting y construcción es exactamente lo que necesitaba nuestro equipo.ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â", initial: "A", name: "Adriana V.", role: "Constructora Residencial", location: "Raleigh, NC", color: "from-purple-400 to-indigo-600" },
      { quote: "ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“Los reportes y punch lists hacen que las conversaciones con contratistas sean más limpias.ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â", initial: "D", name: "Diego L.", role: "Operador BRRR", location: "Orlando, FL", color: "from-amber-300 to-orange-500" },
    ]
    : [
...
function ByNumbers({ isEs }) {
  return <section><SectionHeader title={isEs ? "En Números" : "By the Numbers"} detail={isEs ? "Resultados reales impulsados por herramientas inteligentes." : "Real results powered by intelligent tools."} /><div className="grid gap-4 md:grid-cols-3"><Info title={isEs ? "15% Menos Retrasos" : "15% Fewer Delays"} text={isEs ? "La predicci?n de cuellos de botella con IA identifica riesgos de ruta cr?tica antes de que se conviertan en sobrecostos." : "AI bottleneck prediction flags critical-path risk before it becomes expensive schedule drift."} /><Info title={isEs ? "Precisi?n en Takeoff" : "Takeoff Accuracy"} text={isEs ? "Mediciones automatizadas con factores de desperdicio reducen errores de conteo manual y faltantes de material." : "Automated measurement logic with waste factors reduces manual counting mistakes and material gaps."} /><Info title={isEs ? "2x M?s R?pido en Análisis" : "2x Faster Underwriting"} text={isEs ? "ROI, financiamiento y DSCR en tiempo real convierten horas de hojas de cálculo en minutos." : "Real-time ROI, financing, and DSCR calculations turn spreadsheet hours into minutes."} /></div></section>;
}

function LandingFAQ({ isEs }) {
  const faqs = isEs
    ? [["ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¿Operitron reemplaza a mi contratista o asesor?", "No. OPERITRON.COM organiza cálculos y flujos de trabajo para apoyar decisiones; siempre valida con profesionales licenciados."], ["ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¿Puedo usarlo para flips, rentals y BRRR?", "Sí. Incluye underwriting, DSCR, cash-out, construcción, takeoffs y reportes."], ["ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¿Incluye prueba gratis?", "Sí. La prueba gratis de tres días comienza al iniciar una suscripci?n Mensual o Anual en el checkout seguro."], ["ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¿Puedo colaborar con mi equipo?", "Sí. Puedes estructurar colaboradores, elementos vinculados, cotizaciones y reportes por proyecto."]]
  ...
  return (
    <footer className="rounded-[2rem] border border-white/10 bg-slate-950/70 p-6 shadow-2xl shadow-black/20">
      <div className="grid gap-5 md:grid-cols-3">
        <Info title={isEs ? "15% Menos Retrasos" : "15% Fewer Delays"} text={isEs ? "La predicci?n de cuellos de botella con IA identifica riesgos de ruta cr?tica antes de que se conviertan en sobrecostos de cronograma." : "AI bottleneck prediction identifies critical-path risk before it becomes schedule overrun."} />
        <Info title={isEs ? "Precisi?n en Takeoff" : "Takeoff Accuracy"} text={isEs ? "Mediciones automatizadas con l?gica de factores de desperdicio reducen errores de conteo manual y faltantes de material." : "Automated measurements with waste-factor logic reduce manual counting errors and material shortages."} />
        <Info title={isEs ? "2x M?s R?pido en Análisis" : "2x Faster Analysis"} text={isEs ? "ROI en tiempo real, proyecciones de financiamiento y cálculos DSCR convierten horas de hojas de cálculo en minutos." : "Real-time ROI, financing projections, and DSCR calculations turn spreadsheet hours into minutes."} />
      </div>
      ...
      <div className="mt-8 grid gap-6 border-t border-white/10 pt-6 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <FooterColumn title={isEs ? "M?s" : "More"} items={moreLinks} go={go} />
...
function OwnerConsole({ language, user, go, setActiveTool }) {
  ...
  async function fetchAdminHealth({ production = false, stripe = false } = {}) {
    ...
    try {
      if (!supabase) throw new Error(isEs ? "La autenticaci?n no está configurada." : "Authentication is not configured.");
      const { data } = await supabase.auth.getSession();
      const token = data.session?.access_token;
      ...
      const result = await readApiJson(response);
      if (!response.ok) throw new Error(result.error || (isEs ? "No se pudo verificar producci?n." : "Production check could not be completed."));
      setReport(result);
...
function ToolBody({ t, language, toolId, project }) {
  ...
  if (toolId === "todo") return <Checklist project={project} items={language === "es" ? ["Ordenar armaduras", "Confirmar inspecci?n de cimentaci?n", "Recopilar tres ofertas de HVAC", "Programar cuadrilla de drywall"] : ["Order trusses", "Confirm foundation inspection", "Collect three HVAC bids", "Schedule drywall crew"]} language={language} />;
...
function EmbeddedConstructionProgress({ plan, language }) {
  ...
  return <section className="rounded-3xl border border-white/10 bg-slate-950/60 p-5">
    <div className="flex flex-wrap items-center justify-between gap-4">
      ...
      <MiniMetric label={isEs ? "Duraci?n" : "Duration"} value={`${totalWeeks} ${isEs ? "sem" : "wks"}`} />
    </div>
...
function GeneratedConstructionChecklist({ plan, assumptions, language }) {
  ...
  return <section className="space-y-5 rounded-3xl border border-cyan-300/25 bg-slate-950/70 p-5 sm:p-6">
    ...
    <div className="grid gap-3 sm:grid-cols-4">
      <MiniMetric label={isEs ? "Duraci?n Estimada" : "Estimated Duration"} value={`${plan.totalWeeks} ${isEs ? "semanas" : "weeks"}`} />
...
function SmartConstructionChecklist({ plan, assumptions, language }) {
  ...
  return <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="space-y-5 rounded-3xl border border-cyan-300/25 bg-slate-950/70 p-5 sm:p-6">
    ...
    <div className="rounded-2xl border border-emerald-300/20 bg-emerald-300/[.06] p-4">
      <div className="flex flex-wrap items-center justify-between gap-3"><p className="font-black text-emerald-200">{isEs ? "Duraci?n Total Estimada" : "Total Estimated Duration"}: {totalWeeks} {isEs ? "semanas" : "weeks"}</p><p className="font-black text-cyan-200">{completion}% {isEs ? "completo" : "complete"}</p></div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center">
        <h3 className="text-2xl font-black text-white sm:text-3xl">{isEs ? "Calculadora de Pr?stamos de Inversi?n" : "Investment Loan Calculator"}</h3>
...
function EmbeddedLoanCalculations({ language, mode, metrics }) {
  ...
  return <section className="rounded-3xl border border-white/10 bg-slate-950/60 p-5">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div>
        <p className="text-xs font-black uppercase tracking-widest text-cyan-300">{isEs ? "Cálculos de Pr?stamo" : "Loan Calculations"}</p>
      ...
      {property ? (
        <>
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            ...
            <Stat title={isEs ? "Renta Estimada" : "Rent Estimate"} value={rent ? formatMoney(rent) : t.unavailable} icon={Home} help={isEs ? "Estimaci?n mensual de renta de RentCast." : "Monthly rent estimate from RentCast."} />
          </div>
...
function Tours({ t, language }) {
  ...
  return <div className="grid gap-5 md:grid-cols-3">{titles.map((title, index) => <GlassPanel key={title}><PlayCircle className="text-amber-300" /><p className="mt-5 text-xl font-black text-white">{title}</p><p className="mt-2 text-slate-400">{language === "es" ? `Recorrido guiado ${index + 1} con pasos pr?cticos y puntos de control.` : `Guided tour ${index + 1} with practical steps and checkpoints.`}</p><button className="secondary-button mt-5">{t.startTour}</button></GlassPanel>)}</div>;
}
...
function KnowledgeBase({ t, language }) {
  ...
  return <GlassPanel><h2 className="text-4xl font-black text-white">{t.knowledge}</h2><p className="mt-3 text-slate-400">{language === "es" ? `Respuestas detalladas sobre c?mo ${t.brand} potencia cada fase de la inversión inmobiliaria.` : `Detailed answers about how ${t.brand} supports each phase of real estate investing.`}</p><div className="mt-8 space-y-4">{faqs.map(([q, a]) => <details key={q} className="rounded-3xl border border-white/10 bg-slate-950/70 p-5"><summary className="cursor-pointer text-lg font-black text-white">{q}</summary><p className="mt-4 leading-7 text-slate-300">{a}</p></details>)}</div></GlassPanel>;
}
...
function PricingPlans({ language, user, go }) {
  ...
  const monthlyFeatures = isEs
    ? ["Proyectos y deals ilimitados", "Analizador de deals y fix-and-flip", "Calculadoras DSCR, BRRR y cash-out", "Rastreador de construcción", "Takeoff de materiales con IA", "Punch list", "Gesti?n de subcontratistas y ofertas", "Compartir proyectos y colaborar", "Asistente IA en cada herramienta"]
  ...
  const plans = [
    ...
    { id: "annual", name: isEs ? "Anual" : "Annual", price: "$249.99", cadence: isEs ? "/año" : "/year", note: isEs ? "Ahorra más de 30% ÃƒÆ’Ã¢â‚¬Å¡| Prueba gratis de 3 días" : "Save over 30% ÃƒÆ’Ã¢â‚¬Å¡| 3-day free trial", detail: isEs ? "El mejor valor para operadores activos." : "Best value for active operators.", features: annualFeatures, featured: true },
  ];
...
function PasswordField({ value, onChange, placeholder, autoComplete, visible, onToggle, language }) {
  ...
  return <div>
    ...
    <div className="mt-2 flex flex-wrap items-center gap-2 text-xs font-bold">
      <span className="text-slate-500">{language === "es" ? "Las contraseñas distinguen may?sculas y min?sculas." : "Passwords are case-sensitive."}</span>
      {capsLock && <span className="rounded-full border border-amber-300/30 bg-amber-300/10 px-3 py-1 text-amber-200">{language === "es" ? "Bloq May?s activado" : "Caps Lock is on"}</span>}
    </div>
  </div>;
}
...
function SettingsPage({ t, language, user, setUser, go, back, signOut, passwordRecovery, setPasswordRecovery }) {
  ...
  const ui = isEs
    ? { register: "Registrarse", login: "Iniciar sesión", details: "Crea tu cuenta de Operitron. Las herramientas se activan al iniciar una suscripci?n o prueba mediante Stripe.", recoveryDetails: "Crea una nueva contraseña segura para tu cuenta.", fullName: "Nombre completo", company: "Empresa", phone: "Tel?fono", dashboard: "Volver al panel", signOut: "Cerrar sesión", back: "Volver", save: "Crear cuenta", secure: "Tu cuenta se protege mediante autenticaci?n segura y verificaci?n por correo." }
...
function LegalPage({ type, language }) {
  ...
  const spanish = {
    privacy: { title: "Política de Privacidad", intro: "Esta Política de Privacidad explica c?mo OPERITRON.COM recopila, usa y protege informaci?n al usar nuestro software inmobiliario y de construcción.", sections: [["Informaci?n que Recopilamos", "Podemos recopilar datos de cuenta, correo, empresa, teléfono, actividad de autenticaci?n, proyectos guardados, cálculos, documentos, colaboradores y comunicaciones de soporte."], ["Uso de la Informaci?n", "Usamos la informaci?n para autenticar usuarios, operar espacios de trabajo, guardar análisis, habilitar facturaci?n, brindar soporte y mantener la seguridad del servicio."], ["Pagos y Proveedores", "Stripe procesa los pagos. Supabase puede administrar autenticaci?n y datos guardados. Los proveedores autorizados de datos procesan b?squedas solicitadas por usted bajo sus propias pol?ticas."], ["Seguridad y Retenci?n", "Usamos controles de acceso y pol?ticas a nivel de fila para limitar el acceso de cada usuario a sus propios registros. Ning?n servicio en l?nea garantiza seguridad absoluta."], ["Sus Opciones", "Puede solicitar acceso, correcci?n o eliminaci?n de informaci?n de su cuenta comunic?ndose con soporte de Operitron."]]},
    terms: { title: "Términos de Servicio", intro: "Estos Términos regulan el uso de OPERITRON.COM, una plataforma para inversionistas y operadores de construcción.", sections: [["Responsabilidad de Cuenta", "Debe proporcionar informaci?n precisa, proteger sus credenciales y responsabilizarse por la actividad de su cuenta y colaboradores invitados."], ["Uso Permitido", "Puede usar Operitron para organizar proyectos, analizar inversiones, estimar construcción y preparar reportes. No puede interrumpir el servicio ni intentar acceso no autorizado."], ["Suscripciones y Pruebas", "El acceso pagado se ofrece mediante planes mensuales y anuales con prueba gratis de tres días cuando se muestre en checkout. Stripe controla la facturaci?n aceptada por usted."], ["Datos del Cliente", "Usted conserva propiedad de su contenido y autoriza su procesamiento limitado para prestar el servicio."], ["Limitaci?n de Responsabilidad", "En la medida permitida por ley, Operitron no responde por p?rdidas de inversión, sobrecostos de construcción o resultados de financiamiento basados en resultados de la plataforma."]]},
    refund: { title: "Política de Reembolsos", intro: "Buscamos que la facturaci?n de Operitron sea clara y predecible.", sections: [["Prueba Gratis de Tres D?as", "Las nuevas suscripciones elegibles pueden incluir una prueba de tres días seg?n se muestre en checkout. Cancele antes del fin de la prueba para evitar el primer cargo."], ["Cancelaciones", "Puede cancelar en el portal de facturaci?n. La cancelaci?n evita renovaciones futuras y normalmente mantiene acceso hasta terminar el per?odo pagado actual."], ["Solicitudes de Reembolso", "Los cargos de suscripci?n generalmente no son reembolsables una vez iniciado el per?odo, salvo exigencia legal o error de facturaci?n."], ["Promociones", "Los descuentos se aplican mediante Stripe Checkout y no se cambian por efectivo ni se aplican retroactivamente."]]},
