...
function Header({ t, language, setLanguage, setMobileOpen, go, user, signOut, hasProductAccess, isAdmin, collapsed, openContact }) {
  ...
  async function manageBilling() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error || (language === "es" ? "Primero inicia una suscripci?n." : "Start a subscription first."));
        ...
        <div className="flex shrink-0 items-center gap-2 sm:gap-3">
          ...
          {user ? <div className="relative">
            ...
            {accountOpen && <div className="absolute right-0 top-[calc(100%+0.65rem)] z-50 w-64 rounded-2xl border border-white/10 bg-slate-950 p-2 shadow-2xl shadow-black/50">
              ...
              <button onClick={() => { setAccountOpen(false); manageBilling(); }} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left font-bold text-slate-200 hover:bg-white/5"><WalletCards size={16} />{language === "es" ? "Administrar suscripci?n" : "Manage Subscription"}{(hasProductAccess || isAdmin) && <span className="ml-auto rounded-full bg-emerald-400/15 px-2 py-0.5 text-xs text-emerald-300">{isAdmin ? "Admin" : "Active"}</span>}</button>
...
function FeatureRequestModal({ language, user, onClose }) {
  ...
  return <SupportModalFrame title={isEs ? "Solicitar una funcion" : "Request a Feature"} subtitle={isEs ? "?Tienes una idea para mejorar Operitron? Queremos escucharla." : "Have an idea to make Operitron better? We'd love to hear it."} icon={Sparkles} onClose={onClose}>
    <div className="mt-8 grid gap-5">
      ...
      <label><span className="label">{isEs ? "Describe tu idea" : "Describe your idea"}</span><textarea className="field min-h-52 text-lg" value={message} onChange={(event) => setMessage(event.target.value)} placeholder={isEs ? "?Qu? har?a esta funci?n? ?C?mo ayudar?a?" : "What would this feature do? How would it help you?"} /></label>
    </div>
    {status && <p className="mt-4 rounded-2xl border border-cyan-300/20 bg-cyan-300/10 p-3 text-sm font-bold text-cyan-100">{status}</p>}
...
function PricingPlans({ language, user, go }) {
  ...
  const plans = [
    ...
    { id: "annual", name: isEs ? "Anual" : "Annual", price: "$249.99", cadence: isEs ? "/año" : "/year", note: isEs ? "Ahorra más de 30% ÃƒÆ’Ã¢â‚¬Å¡| Prueba gratis de 3 días" : "Save over 30% | 3-day free trial", detail: isEs ? "El mejor valor para operadores activos." : "Best value for active operators.", features: annualFeatures, featured: true },
  ];
