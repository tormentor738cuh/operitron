...
function subscriptionPlanLabel(plan, language) {
  ...
  if (["no subscription", "inactive", ""].includes(normalized)) return language === "es" ? "Sin suscripciÃ³n" : "No subscription";
  if (normalized === "subscribed") return language === "es" ? "SuscripciÃ³n activa" : "Active subscription";
  return plan;
}
...
const seoArticles = {
  ...
  dscrLoans: {
    ...
    cta: "Use Operitronâ€™s investment loan calculator to compare DSCR, cash-out, and construction loan scenarios.",
  },
  constructionBudgets: {
  ...
  fixFlipAnalysis: {
    ...
    cta: "Use Operitronâ€™s deal underwriter to calculate profit, ROI, 70% rule, and max offer before making an offer.",
  },
  airbnbRoi: {
  ...
  houseBuildCosts: {
    ...
    cta: "Use Operitronâ€™s construction wizard and budget estimator to turn build assumptions into a project checklist and cost model.",
  },
  framingTimelines: {
    ...
    cta: "Use Operitronâ€™s construction progress tools to track framing tasks, critical orders, risk alerts, and schedule dependencies.",
  },
};
...
const copy = {
  ...
  es: {
    ...
    propertySearch: "BÃºsqueda de Propiedad",
    ...
    profile: "Perfil",
    startTrial: "Iniciar prueba gratis de 3 dÃ­as",
    command: "AI Real Estate Operating System",
    ...
    trialNote: "Prueba gratis de 3 dÃ­as. Cancela cuando quieras.",
...
const enhancedCopy = {
  ...
  es: {
    ...
    terms: "TÃ©rminos",
    refund: "PolÃ­tica de Reembolsos",
    ...
    earlyAccessText: "Prueba gratis de 3 dÃ­as para inversionistas, constructores y operadores.",
    myProjects: "Mis Proyectos",
    projectHint: "Haz clic en cualquier proyecto para abrir herramientas, cÃ¡lculos y registros vinculados.",
    createProjectHint: "Crea un deal, construcciÃ³n, flip, renta o espacio de trabajo de obra.",
    projectName: "Nombre del proyecto",
    propertyAddress: "DirecciÃ³n de la propiedad",
    addProject: "Agregar Proyecto",
    heroTitle: "Un espacio original para anÃ¡lisis de deals, registros de propiedades, herramientas de construcciÃ³n y reportes para inversionistas.",
    heroText: "Creado para inversionistas, constructores y operadores que necesitan nÃºmeros claros y ejecuciÃ³n mÃ¡s ordenada.",
    viewLearning: "Ver Centro de Aprendizaje",
    whatsInside: "QuÃ© incluye",
    ...
    loggedIn: "SesiÃ³n iniciada.",
    confirmEmail: "Confirmar correo electrÃ³nico",
    forgotPassword: "Â¿Olvidaste tu contraseÃ±a?",
    resetPassword: "Enviar enlace de recuperaciÃ³n",
    resetSent: "Las instrucciones para restablecer tu contraseÃ±a se enviaron a tu correo.",
    ...
    passwordUpdated: "ContraseÃ±a actualizada. Puedes continuar a tu panel.",
    emailMismatch: "Los correos electrÃ³nicos no coinciden.",
    authUnavailable: "El acceso a cuentas no estÃ¡ disponible en esta vista previa. Escribe a support@operitron.com para recibir ayuda.",
    loginReady: "Acceso seguro a tu cuenta.",
    signingIn: "Iniciando sesiÃ³n...",
    creatingAccount: "Creando cuenta...",
    sessionConfirmed: "Cuenta confirmada. Ya puedes iniciar sesiÃ³n.",
    propertyReady: "Listo para buscar.",
    rentcastKeyRequired: "Los registros de propiedad no estÃ¡n disponibles temporalmente. Contacta support@operitron.com.",
    ...
    owner: "Propietario",
    bedsBaths: "RecÃ¡maras / BaÃ±os",
    squareFeet: "Pies Cuadrados",
    lotSize: "TamaÃ±o del Lote",
    yearBuilt: "AÃ±o de ConstrucciÃ³n",
    lastSale: "Ãšltima Venta",
    coordinates: "Coordenadas",
    assessments: "AvalÃºos Fiscales",
    propertyTaxes: "Impuestos de Propiedad",
    availableAfterSearch: "Disponible despuÃ©s de buscar",
    unavailable: "No disponible",
    aiAssistant: "Asistente IA de Deals",
    aiPrompt: "Revisa este deal por riesgo, oportunidad y prÃ³ximos pasos.",
    aiPreview: "Vista IA: verifica comps, alcance de rehabilitaciÃ³n, impuestos, seguro, permisos, dÃ­as en mercado y tÃ©rminos del prestamista antes de avanzar.",
    aiPreviewNext: "Vista IA: los siguientes pasos mÃ¡s fuertes son validar ventas comparables, revisar permisos, confirmar precios de contratistas y probar el DSCR con una tasa mÃ¡s alta.",
    ...
    propertySearchDetail: "Busca registros de propiedad, comps, propietario, impuestos, historial de ventas, caracterÃ­sticas, coordenadas y cÃ¡lculos para inversionistas.",
    equity: "Equidad",
    ...
    savedProjectsHelp: "NÃºmero de anÃ¡lisis de propiedades guardados en tu espacio de trabajo.",
...
const toolsEs = [
  ["wizard", "Asistente de ConstrucciÃ³n", "Planifica fases y rastrea cronograma, presupuesto, fotos y avance.", Hammer, "Pro"],
  ["underwriter", "Analizador de Deals", "Revisa ARV, reparaciones, oferta, ROI y riesgo.", LineChart, "Core"],
  ["loan", "Calculadora de PrÃ©stamo de InversiÃ³n", "Modela apalancamiento, puntos, pago y DSCR.", Calculator, "DSCR"],
  ["todo", "Lista de Tareas", "Rastrea prÃ³ximos pasos de inversionistas, prestamistas y contratistas.", ListChecks, "Live"],
  ["punch", "Lista de Pendientes", "Registra detalles de inspecciÃ³n final antes del cierre.", ClipboardCheck, "Oficios"],
  ["takeoff", "CÃ¡lculo de Materiales Beta", "Estima cantidades desde Ã¡reas, habitaciones y alcance.", Layers, "Beta"],
    ...
    <header className={`sticky top-0 z-30 border-b border-white/10 bg-slate-950/90 px-3 py-3 backdrop-blur-xl transition-[margin] duration-300 sm:px-5 ${user && hasProductAccess ? (collapsed ? "lg:ml-20" : "lg:ml-72") : ""}`}>
      <div className={`mx-auto flex items-center justify-between gap-2 sm:gap-4 ${user ? "" : "max-w-7xl"}`}>
        ...
        <div className="flex shrink-0 items-center gap-2 sm:gap-3">
          <button onClick={() => setLanguage(language === "en" ? "es" : "en")} aria-label={language === "en" ? "EspaÃ±ol" : "English"} className="flex items-center justify-center gap-2 whitespace-nowrap rounded-xl border border-white/10 px-3 py-2.5 text-sm font-bold text-slate-300 hover:border-cyan-300/50 hover:text-white sm:rounded-2xl sm:px-4 sm:py-3">
            <Languages size={17} /><span className="hidden md:inline">{language === "en" ? "EspaÃ±ol" : "English"}</span><span className="md:hidden">{language === "en" ? "ES" : "EN"}</span>
          </button>
          {user ? <div className="relative">
            ...
            {accountOpen && <div className="absolute right-0 top-[calc(100%+0.65rem)] z-50 w-64 rounded-2xl border border-white/10 bg-slate-950 p-2 shadow-2xl shadow-black/50">
              ...
              <button onClick={() => { setAccountOpen(false); signOut(); }} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left font-bold text-red-300 hover:bg-red-400/10"><LogOut size={16} />{language === "es" ? "Cerrar sesiÃ³n" : "Sign out"}</button>
            </div>}
...
function MobileNavigation({ t, language, activePage, go, user, hasProductAccess }) {
  ...
  return <nav aria-label={isEs ? "NavegaciÃ³n mÃ³vil" : "Mobile navigation"} className="fixed inset-x-3 bottom-3 z-40 grid grid-cols-4 gap-1 rounded-[1.45rem] border border-white/10 bg-slate-950/95 p-1.5 shadow-[0_18px_50px_rgba(0,0,0,.5)] backdrop-blur-xl lg:hidden">{items.map(([page, Icon, label]) => {
...
function Dashboard({ t, language, projects, setProjects, setActiveTool, go, onAddProject, isAdmin }) {
  ...
  async function addProject() {
    ...
    if (onAddProject) {
      ...
      setSaveStatus(result?.error ? (language === "es" ? "Proyecto visible en esta sesiÃ³n, pero no se pudo guardar. Revisa Owner Console." : "Project is visible this session, but could not be saved. Review Owner Console.") : (language === "es" ? "Proyecto guardado." : "Project saved."));
    }
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      {isAdmin && <section className="flex flex-col justify-between gap-4 rounded-3xl border border-cyan-300/25 bg-cyan-300/[.07] p-5 sm:flex-row sm:items-center">
        <div><p className="text-xs font-black uppercase tracking-[0.22em] text-cyan-300">{language === "es" ? "Acceso del Propietario" : "Owner Access"}</p><p className="mt-2 font-bold text-white">{language === "es" ? "Todas las herramientas estÃ¡n habilitadas para tu cuenta administrativa." : "All paid workspace tools are enabled for your administrator account."}</p></div>
      ...
      <section className="grid min-w-0 items-start gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(320px,390px)]">
        <GlassPanel className="self-start">
          ...
          {projects.length ? <div className="grid gap-5 lg:grid-cols-2">
            ...
            <p className="mt-3 font-black text-white">{language === "es" ? "AÃºn no hay proyectos guardados" : "No saved projects yet"}</p>
            <p className="mt-1 text-sm text-slate-400">{language === "es" ? "Crea un proyecto o inicia un nuevo anÃ¡lisis." : "Create a project or begin a new analysis."}</p>
          </div>}
        </GlassPanel>
...
function PublicHome({ t, go }) {
  ...
  const publicCards = isEs
    ? [
      ["AnÃ¡lisis de Deals", "Analiza flips, rentas, BRRR y oportunidades de construcciÃ³n con fÃ³rmulas para inversionistas.", LineChart],
      ["Inteligencia de Propiedades", "Conecta APIs para propietarios, impuestos, historial de ventas, comps y contexto de valuaciÃ³n.", MapPin],
      ["Herramientas de ConstrucciÃ³n", "Gestiona takeoffs, punch lists, cotizaciones, archivos vinculados y colaboradores en un solo espacio.", Hammer],
    ]
    : [
...
function VisitorWorkflow({ isEs }) {
  const steps = isEs
    ? [
      ...
      ["2", "Valida el activo", "Revisa comps, impuestos, datos pÃºblicos, historial y riesgos antes de comprometer capital."],
      ["3", "Planifica la ejecuciÃ³n", "Convierte el alcance en presupuesto, cronograma, takeoff, cotizaciones y punch list."],
  ...
  return <section className="rounded-[2rem] border border-white/10 bg-slate-950/55 p-5 sm:p-7"><SectionHeader title={isEs ? "Del anÃ¡lisis a la ejecuciÃ³n" : "From analysis to execution"} detail={isEs ? "Un flujo claro para inversionistas, constructores y operadores que necesitan verificar antes de actuar." : "A clear operating path for investors and builders who need to verify before they act."} /><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{steps.map(([number, title, text]) => <article key={title} className="rounded-2xl border border-white/10 bg-white/[.035] p-4"><span className="grid h-9 w-9 place-items-center rounded-xl bg-cyan-300/10 font-black text-cyan-300">{number}</span><h3 className="mt-4 text-lg font-black text-white">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-400">{text}</p></article>)}</div><p className="mt-5 rounded-2xl border border-amber-300/15 bg-amber-300/[.05] p-4 text-sm leading-6 text-slate-300">{isEs ? "Operitron apoya decisiones; valida comparables, presupuestos, permisos, financiamiento y asesorÃ­a profesional antes de invertir o construir." : "Operitron supports decisions; verify comps, budgets, permits, financing, and professional advice before investing or building."}</p></section>;
}

function LandingCTA({ isEs, t, go }) {
  return <section className="relative overflow-hidden rounded-[2rem] border border-cyan-300/25 bg-gradient-to-r from-cyan-400/[.10] via-slate-950 to-purple-500/[.10] p-5 shadow-[0_0_42px_rgba(34,211,238,.08)] sm:p-7"><div className="absolute -right-16 -top-16 h-44 w-44 rounded-full bg-cyan-400/15 blur-3xl" /><div className="relative flex flex-col items-start justify-between gap-5 lg:flex-row lg:items-center"><div className="max-w-3xl"><p className="text-xs font-black uppercase tracking-[0.24em] text-cyan-300">{isEs ? "Comienza hoy" : "Start today"}</p><h3 className="mt-3 text-2xl font-black text-white sm:text-3xl">{isEs ? "Â¿Listo para empezar con OPERITRON.COM?" : "Ready to Start with OPERITRON.COM?"}</h3><p className="mt-3 leading-7 text-slate-300">{isEs ? "OPERITRON.COM te ofrece anÃ¡lisis de deals, herramientas de construcciÃ³n, takeoffs, punch lists y colaboraciÃ³n en una sola plataforma. " : "OPERITRON.COM gives you deal analysis, construction tools, takeoffs, punch lists, and collaboration in one platform. "}{t.trialNote}</p></div><div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row"><button onClick={() => go("settings")} className="primary-button whitespace-nowrap">{isEs ? "Crear cuenta" : "Create Account"}</button><button onClick={() => go("pricing")} className="secondary-button whitespace-nowrap">{t.pricing}</button></div></div></section>;
}

function LandingStats({ isEs }) {
  const stats = isEs
    ? [["8", "Herramientas de Proyecto"], ["14", "Fases de ConstrucciÃ³n"], ["2", "Planes Transparentes"], ["3 DÃ­as", "Prueba de Acceso Completo"]]
...
function LandingFeatureSections({ isEs }) {
  const sections = isEs
    ? [
      ["Analiza Deals en Minutos", "EvalÃºa ARV, precio de compra, reparaciones, financiamiento, DSCR, ROI y flujo de caja sin vivir en hojas de cÃ¡lculo.", ["ROI en tiempo real", "Regla del 70%", "Oferta mÃ¡xima", "Escenarios de salida"], LineChart],
      ["Rastrea Cada Fase de ConstrucciÃ³n", "Organiza cronogramas, presupuestos, fotos, inspecciones y dependencias crÃ­ticas desde una sola vista de obra.", ["Fases de construcciÃ³n", "Riesgo de retrasos", "Control de presupuesto", "Progreso por hitos"], BarChart3],
      ["Mide Planos con IA", "Sube planos, estima cantidades, aplica factores de desperdicio y genera reportes de materiales para cotizar con mÃ¡s claridad.", ["PDFs multipÃ¡gina", "Ãreas y lineales", "Costos de materiales", "ExportaciÃ³n PDF"], Layers],
      ["Cierra Proyectos con Punch Lists", "Documenta pendientes, asigna oficios, adjunta fotos y exporta reportes limpios para contratistas, compradores o prestamistas.", ["Fotos por partida", "AsignaciÃ³n por oficio", "Estados abiertos/resueltos", "Reporte PDF"], ClipboardCheck],
    ]
    : [
...
function WhyChoose({ isEs }) {
  const items = isEs
    ? [["NÃºmeros Claros", "FÃ³rmulas visibles para revisar supuestos antes de comprar."], ["Flujo Todo-en-Uno", "Deals, construcciÃ³n, takeoffs y reportes sin cambiar de sistema."], ["Listo para Equipos", "Colabora con socios, contratistas, prestamistas y gerentes."], ["DiseÃ±ado para Operadores", "Creado para decisiones reales de inversiÃ³n y obra, no solo reportes bonitos."]]
  ...
  return <section><SectionHeader title="Why Choose OPERITRON.COM?" detail={isEs ? "IA prÃ¡ctica, cÃ¡lculos claros y control de proyecto en una plataforma original." : "Practical AI, clear calculations, and project control in one original platform."} /><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">{items.map(([title, text]) => <Info key={title} title={title} text={text} />)}</div></section>;
}

function Testimonials({ isEs }) {
  const quotes = isEs
    ? [
      { quote: "â€œOperitron nos ayudÃ³ a revisar mÃ¡s deals sin perder control de los supuestos.â€", initial: "C", name: "Camila S.", role: "Desarrolladora de Vivienda", location: "Phoenix, AZ", color: "from-cyan-400 to-blue-600" },
      { quote: "â€œLa combinaciÃ³n de underwriting y construcciÃ³n es exactamente lo que necesitaba nuestro equipo.â€", initial: "A", name: "Adriana V.", role: "Constructora Residencial", location: "Raleigh, NC", color: "from-purple-400 to-indigo-600" },
      { quote: "â€œLos reportes y punch lists hacen que las conversaciones con contratistas sean mÃ¡s limpias.â€", initial: "D", name: "Diego L.", role: "Operador BRRR", location: "Orlando, FL", color: "from-amber-300 to-orange-500" },
    ]
    : [
      { quote: "â€œOperitron helps us review more deals without losing control of the assumptions.â€", initial: "C", name: "Camila S.", role: "Housing Developer", location: "Phoenix, AZ", color: "from-cyan-400 to-blue-600" },
      { quote: "â€œThe underwriting plus construction workflow is exactly what our team needed.â€", initial: "A", name: "Alyssa V.", role: "Residential Builder", location: "Raleigh, NC", color: "from-purple-400 to-indigo-600" },
      { quote: "â€œReports and punch lists make contractor conversations cleaner.â€", initial: "D", name: "Daniel L.", role: "BRRR Portfolio Operator", location: "Orlando, FL", color: "from-amber-300 to-orange-500" },
    ];
  return <section><SectionHeader title={isEs ? "Confiado por Operadores" : "Trusted by Investors"} detail={isEs ? "Software para equipos que viven entre nÃºmeros, obra y ejecuciÃ³n." : "Software for teams living between numbers, jobsites, and execution."} /><div className="grid gap-4 md:grid-cols-3">{quotes.map((review) => <motion.article key={review.name} whileHover={{ y: -4 }} className="rounded-3xl border border-white/10 bg-slate-950/70 p-5 transition hover:border-cyan-300/25 hover:shadow-[0_16px_45px_rgba(34,211,238,.08)]"><p className="text-amber-300" aria-label="5 out of 5 stars">â˜…â˜…â˜…â˜…â˜…</p><p className="mt-4 min-h-24 leading-7 text-slate-300">{review.quote}</p><div className="mt-5 flex items-center gap-3 border-t border-white/10 pt-4"><div className={`grid h-12 w-12 shrink-0 place-items-center rounded-full bg-gradient-to-br ${review.color} text-lg font-black text-white shadow-[0_0_22px_rgba(34,211,238,.18)]`}>{review.initial}</div><div><p className="font-black text-white">{review.name}</p><p className="text-sm text-cyan-200">{review.role} <span className="text-slate-500">Â·</span> {review.location}</p></div></div></motion.article>)}</div></section>;
}

function ByNumbers({ isEs }) {
  return <section><SectionHeader title={isEs ? "En NÃºmeros" : "By the Numbers"} detail={isEs ? "Resultados reales impulsados por herramientas inteligentes." : "Real results powered by intelligent tools."} /><div className="grid gap-4 md:grid-cols-3"><Info title={isEs ? "15% Menos Retrasos" : "15% Fewer Delays"} text={isEs ? "La predicciÃ³n de cuellos de botella con IA identifica riesgos de ruta crÃ­tica antes de que se conviertan en sobrecostos." : "AI bottleneck prediction flags critical-path risk before it becomes expensive schedule drift."} /><Info title={isEs ? "PrecisiÃ³n en Takeoff" : "Takeoff Accuracy"} text={isEs ? "Mediciones automatizadas con factores de desperdicio reducen errores de conteo manual y faltantes de material." : "Automated measurement logic with waste factors reduces manual counting mistakes and material gaps."} /><Info title={isEs ? "2x MÃ¡s RÃ¡pido en AnÃ¡lisis" : "2x Faster Underwriting"} text={isEs ? "ROI, financiamiento y DSCR en tiempo real convierten horas de hojas de cÃ¡lculo en minutos." : "Real-time ROI, financing, and DSCR calculations turn spreadsheet hours into minutes."} /></div></section>;
}

function LandingFAQ({ isEs }) {
  const faqs = isEs
    ? [["Â¿Operitron reemplaza a mi contratista o asesor?", "No. OPERITRON.COM organiza cÃ¡lculos y flujos de trabajo para apoyar decisiones; siempre valida con profesionales licenciados."], ["Â¿Puedo usarlo para flips, rentals y BRRR?", "SÃ­. Incluye underwriting, DSCR, cash-out, construcciÃ³n, takeoffs y reportes."], ["Â¿Incluye prueba gratis?", "SÃ­. La prueba gratis de tres dÃ­as comienza al iniciar una suscripciÃ³n Mensual o Anual en el checkout seguro."], ["Â¿Puedo colaborar con mi equipo?", "SÃ­. Puedes estructurar colaboradores, elementos vinculados, cotizaciones y reportes por proyecto."]]
  ...
  return <section><SectionHeader title={isEs ? "Preguntas Frecuentes" : "Frequently Asked Questions"} detail={isEs ? "Respuestas rÃ¡pidas para inversionistas y constructores." : "Quick answers for investors and builders."} /><div className="space-y-3">{faqs.map(([q, a]) => <details key={q} className="rounded-3xl border border-white/10 bg-slate-950/70 p-5"><summary className="cursor-pointer font-black text-white">{q}</summary><p className="mt-3 leading-7 text-slate-400">{a}</p></details>)}</div></section>;
}

function LandingKnowledgeBase({ isEs }) {
  const topics = isEs ? ["CÃ³mo evaluar un deal mÃ¡s rÃ¡pido", "CÃ³mo preparar una oferta mÃ¡xima", "CÃ³mo organizar un cierre de construcciÃ³n", "CÃ³mo revisar DSCR antes de hablar con un prestamista"] : ["How to evaluate a deal faster", "How to prepare a max offer", "How to organize construction closeout", "How to review DSCR before calling a lender"];
  return <section><SectionHeader title={isEs ? "Base de Conocimiento para Inversionistas" : "Investor Knowledge Base"} detail={isEs ? "GuÃ­as originales para analizar, construir y operar con mÃ¡s claridad." : "Original guides for analyzing, building, and operating with more clarity."} /><div className="grid gap-4 md:grid-cols-2">{topics.map((topic) => <button key={topic} className="rounded-3xl border border-white/10 bg-slate-950/70 p-5 text-left font-black text-white hover:border-cyan-300/40">{topic}<ChevronRight className="float-right text-cyan-300" /></button>)}</div></section>;
}
  ...
  return (
    <article className="mx-auto max-w-6xl space-y-8">
      <section className="relative overflow-hidden rounded-[2rem] border border-cyan-300/20 bg-gradient-to-br from-cyan-400/[.12] via-slate-950 to-purple-500/[.12] p-6 shadow-2xl shadow-black/25 sm:p-10">
        ...
        <button onClick={() => go("home")} className="secondary-button relative mb-6">â† Home</button>
...
function Hero({ t, go }) {
  ...
  const insideItems = isEs
    ? ["Analizador de Deals", "Calculadora DSCR", "Calculadora BRRR", "Rastreador de ConstrucciÃ³n", "Takeoff de Materiales con IA", "Punch List", "Checklist de ConstrucciÃ³n", "ColaboraciÃ³n de Equipo"]
...
function PublicFooter({ isEs, go }) {
  const toolLinks = isEs ? [["AnÃ¡lisis de Deals"], ["Calculadora DSCR"], ["Calculadora BRRR"], ["Rastreador de ConstrucciÃ³n"]] : [["Deal Analysis"], ["DSCR Calculator"], ["BRRR Calculator"], ["Construction Tracker"]];
  ...
  const companyLinks = isEs ? [["TÃ©rminos", "terms"], ["Privacidad", "privacy"], ["Reembolsos", "refund"], ["Aviso Legal", "disclaimer"]] : [["Terms", "terms"], ["Privacy", "privacy"], ["Refunds", "refund"], ["Disclaimer", "disclaimer"]];
  return (
    <footer className="rounded-[2rem] border border-white/10 bg-slate-950/70 p-6 shadow-2xl shadow-black/20">
      <div className="grid gap-5 md:grid-cols-3">
        <Info title={isEs ? "15% Menos Retrasos" : "15% Fewer Delays"} text={isEs ? "La predicciÃ³n de cuellos de botella con IA identifica riesgos de ruta crÃ­tica antes de que se conviertan en sobrecostos de cronograma." : "AI bottleneck prediction identifies critical-path risk before it becomes schedule overrun."} />
        <Info title={isEs ? "PrecisiÃ³n en Takeoff" : "Takeoff Accuracy"} text={isEs ? "Mediciones automatizadas con lÃ³gica de factores de desperdicio reducen errores de conteo manual y faltantes de material." : "Automated measurements with waste-factor logic reduce manual counting errors and material shortages."} />
        <Info title={isEs ? "2x MÃ¡s RÃ¡pido en AnÃ¡lisis" : "2x Faster Analysis"} text={isEs ? "ROI en tiempo real, proyecciones de financiamiento y cÃ¡lculos DSCR convierten horas de hojas de cÃ¡lculo en minutos." : "Real-time ROI, financing projections, and DSCR calculations turn spreadsheet hours into minutes."} />
      </div>
      <div className="mt-6 rounded-3xl border border-amber-400/20 bg-amber-400/10 p-5">
        <p className="font-black text-amber-200">{isEs ? "âš ï¸ Aviso Importante" : "âš ï¸ Important Notice"}</p>
        <p className="mt-2 leading-7 text-slate-300">{isEs ? "Importante: Operitron es una herramienta de apoyo a la toma de decisiones, no una garantÃ­a. Todas las proyecciones, presupuestos y rendimientos son estimaciones basadas en sus datos y suposiciones. Operitron no es responsable de pÃ©rdidas, retrasos, sobrecostos, problemas de cumplimiento de cÃ³digos, resultados de financiamiento o resultados de inversiÃ³n. Siempre verifique los datos y consulte a profesionales licenciados." : "Operitron is decision-support software, not a guarantee. All projections, budgets, and returns are estimates based on your inputs and assumptions. Always verify data and consult licensed professionals."}</p>
      </div>
      <div className="mt-8 grid gap-6 border-t border-white/10 pt-6 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <FooterColumn title={isEs ? "MÃ¡s" : "More"} items={moreLinks} go={go} />
      ...
      <div className="mt-8 flex flex-wrap items-center justify-between gap-3 border-t border-white/10 pt-5 text-sm text-slate-500">
        <p>Â© 2026 Operitron. {isEs ? "Todos los derechos reservados." : "All rights reserved."}</p>
    ...
    return (
      <div className="mx-auto mt-6 max-w-xl rounded-[2rem] border border-white/10 bg-white/[.035] p-5 shadow-2xl shadow-black/30 sm:p-7">
        ...
        <label className="mt-6 block">
          ...
          <input autoFocus value={projectName} onChange={(event) => setProjectName(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") createProject(); }} className="field text-base" placeholder={isEs ? "Ej. RemodelaciÃ³n Silva" : "Example: Silva Remodel"} />
        </label>
        <label className="mt-4 block">
          <span className="label">{isEs ? "DescripciÃ³n (opcional)" : "Description (optional)"}</span>
          <textarea value={description} onChange={(event) => setDescription(event.target.value)} className="field min-h-24 resize-y text-base" placeholder={isEs ? "Notas, direcciÃ³n, alcance o contexto..." : "Notes, address, scope, or context..."} />
        </label>
      ...
      {!showExisting ? <div className="mt-7 grid gap-3">
        ...
        <div className="max-h-72 space-y-3 overflow-y-auto pr-2 [scrollbar-color:rgba(34,211,238,.55)_rgba(15,23,42,.75)] [scrollbar-width:thin]">
          {filteredProjects.length ? filteredProjects.map((project) => <button key={project.id} onClick={() => selectProject(project)} className="glow-card flex w-full items-center gap-4 rounded-2xl border border-white/10 bg-slate-950/60 p-4 text-left hover:border-cyan-300/35">
            ...
            <span className="min-w-0 flex-1"><span className="block truncate font-black text-white">{project.name || project.title || "Untitled Project"}</span><span className="block truncate text-sm text-slate-400">{project.description || project.address || project.notes || (isEs ? "Sin descripciÃ³n" : "No description")}</span></span>
            <ChevronRight className="shrink-0 text-cyan-300" size={18} />
          </button>) : <div className="rounded-2xl border border-dashed border-white/10 bg-slate-950/40 p-5 text-center text-sm text-slate-400">{isEs ? "No encontramos proyectos con esa bÃºsqueda." : "No projects match that search."}</div>}
        </div>
...
function OwnerConsole({ language, user, go, setActiveTool }) {
  ...
  async function fetchAdminHealth({ production = false, stripe = false } = {}) {
    ...
    try {
      if (!supabase) throw new Error(isEs ? "La autenticaciÃ³n no estÃ¡ configurada." : "Authentication is not configured.");
      const { data } = await supabase.auth.getSession();
      const token = data.session?.access_token;
      if (!token) throw new Error(isEs ? "Inicia sesiÃ³n para continuar." : "Sign in to continue.");
      const response = await fetch("/api/admin-health", { headers: { Authorization: `Bearer ${token}` } });
      const result = await readApiJson(response);
      if (!response.ok) throw new Error(result.error || (isEs ? "No se pudo verificar producciÃ³n." : "Production check could not be completed."));
      setReport(result);
      if (stripe) setStripeStatus(isEs ? "ConfiguraciÃ³n de Stripe verificada. No se creÃ³ ningÃºn cargo." : "Stripe configuration checked. No charge was created.");
    } catch (error) {
  ...
  return <div className="space-y-6">
    <section className="relative overflow-hidden rounded-[2rem] border border-cyan-300/25 bg-gradient-to-br from-cyan-400/[.12] via-slate-950 to-purple-500/[.12] p-6 shadow-[0_0_48px_rgba(34,211,238,.1)] sm:p-8">
      ...
      <div className="relative flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
        <div>
          ...
          <p className="mt-3 max-w-2xl leading-7 text-slate-300">{isEs ? "Tu identidad administrativa omite el cobro para desarrollo, revisiÃ³n y control de todas las herramientas del producto." : "Your administrator identity bypasses billing for product development, review, and control across every workspace tool."}</p>
          <p className="mt-3 inline-flex rounded-full border border-cyan-300/25 bg-cyan-300/10 px-4 py-2 text-sm font-black text-cyan-200">{user?.email} Â· {isEs ? "Administrador" : "Administrator"}</p>
        </div>
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
      <GlassPanel>
        <SectionHeader title={isEs ? "Acceso a Herramientas" : "Tool Access"} detail={isEs ? "Las ocho Ã¡reas profesionales estÃ¡n desbloqueadas para tu cuenta administrativa." : "All eight professional work areas are unlocked for your administrator account."} />
      ...
      <GlassPanel>
        <SectionHeader title={isEs ? "Estado de ProducciÃ³n" : "Production Readiness"} detail={isEs ? "Comprobaciones privadas para detectar configuraciÃ³n faltante." : "Private checks to surface missing configuration."} />
        ...
        {issues.length > 0 && <p className="mt-4 rounded-xl border border-amber-300/20 bg-amber-300/[.06] p-3 text-sm leading-6 text-amber-100">{isEs ? "Los elementos en amarillo requieren configuraciÃ³n antes de guardar datos o procesar servicios conectados." : "Yellow items require configuration before related data can save or connected services can run."}</p>}
      </GlassPanel>
    </div>

    <GlassPanel>
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
        <SectionHeader title={isEs ? "Checklist de Lanzamiento" : "Launch Checklist"} detail={isEs ? "VerificaciÃ³n segura y de solo lectura. No crea checkout ni cobra tarjetas." : "Safe read-only verification. It does not create checkout sessions or charge cards."} />
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2">
        {(stripeChecks.length ? stripeChecks : [
          ...
          <p className={`flex items-center gap-2 text-sm font-black ${check.ok ? "text-emerald-300" : "text-red-200"}`}>{check.ok ? <CheckCircle2 size={17} /> : <X size={17} />}{check.ok ? "PASS" : "FAIL"} Â· {check.name}</p>
...
function PremiumPaywall({ language, user, go }) {
  const isEs = language === "es";
  const perks = isEs
    ? ["Analizador IA y underwriting avanzado", "Calculadoras DSCR, BRRR y cash-out", "Proyectos, reportes, Dropbox y colaboraciÃ³n", "Rastreo de construcciÃ³n y takeoff de materiales"]
...
function ToolBody({ t, language, toolId, project }) {
  ...
  if (toolId === "todo") return <Checklist project={project} items={language === "es" ? ["Ordenar armaduras", "Confirmar inspecciÃ³n de cimentaciÃ³n", "Recopilar tres ofertas de HVAC", "Programar cuadrilla de drywall"] : ["Order trusses", "Confirm foundation inspection", "Collect three HVAC bids", "Schedule drywall crew"]} language={language} />;
  ...
  return <section className="rounded-3xl border border-white/10 bg-slate-950/60 p-5">
    <div className="flex flex-wrap items-center justify-between gap-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h4 className="text-xl font-black text-white">{isEs ? "Progreso de ConstrucciÃ³n" : "Construction Progress"}</h4>
      ...
      <MiniMetric label={isEs ? "DuraciÃ³n" : "Duration"} value={`${totalWeeks} ${isEs ? "sem" : "wks"}`} />
    </div>
  ...
  return <section className="space-y-5 rounded-3xl border border-cyan-300/25 bg-slate-950/70 p-5 sm:p-6">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div>
        ...
        <h4 className="mt-2 text-2xl font-black text-white">{isEs ? "Tu Lista de ConstrucciÃ³n" : "Your Construction Checklist"}</h4>
    ...
    <div className="grid gap-3 sm:grid-cols-4">
      <MiniMetric label={isEs ? "DuraciÃ³n Estimada" : "Estimated Duration"} value={`${plan.totalWeeks} ${isEs ? "semanas" : "weeks"}`} />
      <MiniMetric label={isEs ? "Costo ConstrucciÃ³n" : "Construction Cost"} value={formatMoney(plan.estimatedCost)} />
    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_300px]">
      ...
      <div className="space-y-4">
        <Info title={isEs ? "RecomendaciÃ³n de IA" : "AI Recommendation"} text={plan.risk === "Moderate" ? (isEs ? "Confirma permisos y condiciones del terreno antes de comprometer cuadrillas." : "Confirm permitting and site conditions before committing crews.") : (isEs ? "El alcance estÃ¡ preparado para programaciÃ³n inicial y cotizaciones." : "Scope is ready for initial scheduling and quote collection.")} />
...
function SmartConstructionChecklist({ plan, assumptions, language }) {
  ...
  async function analyzeWeather() {
    ...
    try {
      ...
      setWeather({ error: isEs ? "No se pudo obtener el pronÃ³stico para esta ubicaciÃ³n." : "Forecast could not be found for this location." });
    } finally {
  ...
  return <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="space-y-5 rounded-3xl border border-cyan-300/25 bg-slate-950/70 p-5 sm:p-6">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div><p className="text-xs font-black uppercase tracking-widest text-cyan-300">{isEs ? "Plan Generado" : "Generated Plan"}</p><h4 className="mt-2 text-2xl font-black text-white">{isEs ? "Tu Lista de ConstrucciÃ³n" : "Your Construction Checklist"}</h4><p className="mt-2 text-sm text-slate-400">{isEs ? "Plan operativo de 14 fases con recomendaciones y seguimiento." : "A 14-phase operating plan with recommendations and live tracking."}</p><p className="mt-2 text-xs text-slate-500">{isEs ? "Ultima actualizacion" : "Last updated"}: {new Date().toLocaleTimeString()}</p></div>
      <button className="secondary-button">{isEs ? "Guardar en Proyecto" : "Save to Project"}</button>
    </div>
    <div className="rounded-2xl border border-emerald-300/20 bg-emerald-300/[.06] p-4">
      <div className="flex flex-wrap items-center justify-between gap-3"><p className="font-black text-emerald-200">{isEs ? "DuraciÃ³n Total Estimada" : "Total Estimated Duration"}: {totalWeeks} {isEs ? "semanas" : "weeks"}</p><p className="font-black text-cyan-200">{completion}% {isEs ? "completo" : "complete"}</p></div>
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
      ...
      <div className="space-y-4">
        ...
        <section className="rounded-2xl border border-white/10 bg-white/[.03] p-4"><h5 className="font-black text-white">{isEs ? "Ã“rdenes de Material" : "Material Orders"}</h5>{orders.map(([title, lead, due]) => <div key={title} className="mt-3 rounded-xl bg-slate-950/60 p-3"><p className="font-bold text-white">{title}</p><p className="mt-1 text-xs text-slate-400">Lead time: {lead}</p><p className="mt-2 text-xs font-black text-cyan-300">Order by: {due}</p></div>)}</section>
      </div>
    </div>
    <section className="rounded-3xl border border-cyan-300/20 bg-white/[.03] p-5"><div className="flex flex-wrap justify-between gap-3"><h5 className="flex items-center gap-2 text-lg font-black text-white"><Globe2 className="text-cyan-300" />{isEs ? "AnÃ¡lisis ClimÃ¡tico" : "Weather Risk Analysis"}</h5>{weather?.time && <p className="text-xs text-slate-500">{isEs ? "Actualizado" : "Updated"}: {weather.time}</p>}</div><div className="mt-4 flex flex-col gap-3 sm:flex-row"><input className="field" value={location} onChange={(event) => setLocation(event.target.value)} placeholder={isEs ? "Buscar ciudad o direcciÃ³n..." : "Search city or address..."} /><button onClick={analyzeWeather} disabled={weatherLoading} className="primary-button shrink-0">{weatherLoading ? (isEs ? "Analizando..." : "Analyzing...") : (isEs ? "Analizar" : "Analyze")}</button></div>{!weather && <p className="mt-3 text-sm text-slate-400">{isEs ? "Configura la ubicaciÃ³n y analiza riesgos con pronÃ³stico de siete dÃ­as." : "Set the project location and analyze risks with a seven-day forecast."}</p>}{weather?.error && <p className="mt-4 rounded-xl border border-rose-300/20 bg-rose-300/10 p-3 text-sm text-rose-200">{weather.error}</p>}{weather && !weather.error && <div className="mt-4 grid gap-3 sm:grid-cols-4"><MiniMetric label={isEs ? "UbicaciÃ³n" : "Location"} value={weather.place} /><MiniMetric label={isEs ? "Riesgo" : "Risk"} value={weather.severity} green={weather.severity === "LOW"} /><MiniMetric label={isEs ? "Prob. lluvia" : "Rain Chance"} value={`${weather.rain}%`} /><MiniMetric label={isEs ? "RÃ¡faga mÃ¡x." : "Max Wind Gust"} value={`${weather.wind} km/h`} /></div>}</section>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center">
        <h3 className="text-2xl font-black text-white sm:text-3xl">{isEs ? "Calculadora de PrÃ©stamos de InversiÃ³n" : "Investment Loan Calculator"}</h3>
        <p className="mx-auto mt-2 max-w-2xl text-sm leading-6 text-slate-400">{isEs ? "Calcula pagos para prÃ©stamos DSCR, refinanciamiento cash-out y construcciÃ³n desde cero." : "Calculate payments for DSCR, Cash-Out Refinance, and Ground-Up Construction loans."}</p>
      </div>
      <div className="mx-auto grid max-w-lg grid-cols-3 rounded-2xl bg-slate-900/80 p-1">
        {[["dscr", "DSCR"], ["cashout", "Cash-Out"], ["ground", isEs ? "ConstrucciÃ³n" : "Ground Up"]].map(([id, label]) => <button key={id} onClick={() => setMode(id)} className={`rounded-xl px-3 py-3 text-sm font-black transition ${mode === id ? "bg-cyan-300 text-slate-950 shadow-[0_0_22px_rgba(34,211,238,.25)]" : "text-slate-400 hover:text-white"}`}>{label}</button>)}
      </div>
      <div className="grid gap-5 xl:grid-cols-2">
        <section className="rounded-3xl border border-white/10 bg-slate-950/60 p-5 sm:p-6">
          <div className="mb-6 flex items-start justify-between gap-4">
            <div><h4 className="text-xl font-black text-white">{mode === "dscr" ? (isEs ? "Calculadora DSCR" : "DSCR Loan Calculator") : mode === "cashout" ? (isEs ? "Calculadora de Refinanciamiento Cash-Out" : "Cash-Out Refinance Calculator") : (isEs ? "Calculadora de InterÃ©s de ConstrucciÃ³n" : "Construction Interest Calculator")}</h4>{mode === "ground" && <p className="mt-1 text-sm text-slate-400">{isEs ? "ConstrucciÃ³n desde cero" : "Ground-Up Construction"}</p>}</div>
...
function EmbeddedLoanCalculations({ language, mode, metrics }) {
  ...
  return <section className="rounded-3xl border border-white/10 bg-slate-950/60 p-5">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div>
        <p className="text-xs font-black uppercase tracking-widest text-cyan-300">{isEs ? "CÃ¡lculos de PrÃ©stamo" : "Loan Calculations"}</p>
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">{isEs ? "Esta secciÃ³n integra los cÃ¡lculos de prÃ©stamo dentro de la calculadora principal para mantener DSCR, cash-out y construcciÃ³n en un solo lugar." : "This folds the old loan-calculations tool into the main calculator so DSCR, cash-out, and construction debt assumptions stay together."}</p>
      </div>
    ...
    <div className="space-y-6">
      <GlassPanel>
        ...
        <div className="mt-6 grid gap-4 xl:grid-cols-[minmax(0,1fr)_260px_170px]">
          <label className="block">
            <span className="label">{isEs ? "DirecciÃ³n" : "Address"}</span>
      ...
      {property ? (
        <>
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            ...
            <Stat title={isEs ? "Renta Estimada" : "Rent Estimate"} value={rent ? formatMoney(rent) : t.unavailable} icon={Home} help={isEs ? "EstimaciÃ³n mensual de renta de RentCast." : "Monthly rent estimate from RentCast."} />
          </div>
          ...
          <GlassPanel>
            <SectionHeader title={isEs ? "CaracterÃ­sticas y Registros" : "Features & Records"} detail={isEs ? "Datos estructurados del registro de propiedad devuelto por RentCast." : "Structured property record details returned by RentCast."} />
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {features.length ? features.map(([label, value]) => <MiniMetric key={label} label={label} value={value} />) : <MiniMetric label={isEs ? "CaracterÃ­sticas" : "Features"} value={t.unavailable} />}
            </div>
          </GlassPanel>
        </>
      ) : (
        <div className="rounded-[2rem] border border-dashed border-white/10 bg-white/[.035] p-10 text-center">
          ...
          <p className="mx-auto mt-2 max-w-2xl text-slate-400">{isEs ? "Los datos de RentCast se consultan en el servidor para mantener la llave segura y se muestran aquÃ­ para anÃ¡lisis de inversiÃ³n." : "RentCast data is requested on the server to keep the key secure, then displayed here for investor-grade review."}</p>
        </div>
      )}
    </div>
  );
      ...
      {items.length ? items.slice(0, 8).map((item, index) => (
        <div key={`${item.address}-${index}`} className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-white/10 bg-slate-950/60 p-4">
          <div>
            ...
            <p className="text-sm text-slate-400">{[item.date, item.distance && `${item.distance} mi`, item.sqft && `${formatNumber(item.sqft, 0)} sqft`, item.beds && `${item.beds} bd`].filter(Boolean).join(" Â· ")}</p>
          </div>
...
function AITakeoff({ language = "en", project }) {
  ...
  return (
    <ToolShell title={isEs ? "CÃ¡lculo de Materiales con IA" : "AI Material Takeoff"} subtitle={isEs ? "Sube planos, ingresa medidas y precios, y exporta un informe profesional." : "Upload plans, enter dimensions and unit prices, then export a polished material takeoff report."}>
      <ProjectContext project={project} language={language} />
      <div className="mb-6 mt-5 flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex gap-3"><MiniMetric label={isEs ? "Planos" : "Sheets"} value={fileName ? "1" : "0"} /><MiniMetric label={isEs ? "CÃ¡lculos" : "Runs"} value={fileName ? "1" : "0"} /></div>
      </div>
...
function PunchListApp({ language = "en", project }) {
  ...
  const [assumptions, setAssumptions] = useState({ salePrice: "365000", purchase: "190000", repairs: "45000", rent: "2800", expenses: "650", cashInvested: "65000", loan: "240000", rate: "7.25", years: "30", adr: "225", booked: "220", available: "365", acres: "2.5", units: "8", lotSize: "10000", footprint: "2600", wallArea: "6200", roofArea: "2800" });
  ...
  const formulaGroups = {
    ...
    construction: [
      ...
      ["Concrete Yards", "Length x Width x Thickness(ft) / 27", `${formatNumber((40 * 30 * 0.33) / 27, 2)} yd³`],
  ...
  const label = language === "es"
    ? { review: "Revisar CotizaciÃ³n", trade: "Oficio", vendor: "Proveedor", bid: "Monto de Oferta", scope: "Alcance", status: "Estado", quoteStatus: "Estado de cotizaciÃ³n", pending: "Pendiente", approved: "Aprobada", rejected: "Rechazada", total: "Total de Ofertas", contingency: "Contingencia 10%", budget: "Presupuesto con Contingencia", selected: "Oferta Seleccionada" }
...
function Tours({ t, language }) {
  const titles = language === "es" ? ["Analiza un flip en 5 minutos", "Configura un proyecto de construcciÃ³n", "Exporta tu primer reporte para inversionistas"] : ["Analyze a flip in 5 minutes", "Set up a construction project", "Export your first investor report"];
  return <div className="grid gap-5 md:grid-cols-3">{titles.map((title, index) => <GlassPanel key={title}><PlayCircle className="text-amber-300" /><p className="mt-5 text-xl font-black text-white">{title}</p><p className="mt-2 text-slate-400">{language === "es" ? `Recorrido guiado ${index + 1} con pasos prÃ¡cticos y puntos de control.` : `Guided tour ${index + 1} with practical steps and checkpoints.`}</p><button className="secondary-button mt-5">{t.startTour}</button></GlassPanel>)}</div>;
}

function Tutorials({ language }) {
  const isEs = language === "es";
  const groups = isEs ? [["General", ["Comenzando con Operitron", "Crear y organizar proyectos"]], ["AnÃ¡lisis de Deals", ["Analizador de Deals: vista general", "Analizar un flip", "AnÃ¡lisis avanzado"]], ["ConstrucciÃ³n", ["Gestionar subcontratistas", "Lista de pendientes", "Asistente de construcciÃ³n"]], ["Takeoffs", ["Configurar el mÃ³dulo de cÃ¡lculo", "Exportar el informe PDF"]]] : [["General", ["Getting Started with Operitron", "Create and organize projects"]], ["Deal Underwriter", ["Deal Underwriter overview", "Analyze a flip", "Advanced deal analysis"]], ["Construction Tools", ["Managing subcontractors", "Punch lists", "Construction wizard"]], ["Takeoffs", ["Configure material takeoff", "Export PDF report"]]];
  return <div className="space-y-7"><div><h2 className="text-3xl font-black text-white">{isEs ? "Tutoriales" : "Tutorials"}</h2><p className="mt-2 text-slate-400">{isEs ? "Aprende a obtener el mÃ¡ximo valor de cada herramienta." : "Learn how to get the most out of every Operitron workflow."}</p></div>{groups.map(([group, videos]) => <section key={group}><h3 className="mb-4 text-lg font-black text-cyan-200">{group}</h3><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{videos.map((title, index) => <button key={title} className="overflow-hidden rounded-3xl border border-white/10 bg-slate-950/60 text-left hover:border-cyan-300/35"><div className="grid h-32 place-items-center bg-gradient-to-br from-blue-500/15 to-purple-500/10"><PlayCircle className="text-cyan-300" size={38} /></div><div className="p-4"><p className="font-black text-white">{title}</p><p className="mt-2 text-xs text-slate-500">{index === videos.length - 1 ? (isEs ? "PrÃ³ximamente" : "Coming Soon") : (isEs ? "Ver tutorial" : "Watch tutorial")}</p></div></button>)}</div></section>)}</div>;
}

function LearningCenter({ t, language, go }) {
  const articles = language === "es"
    ? [["AnÃ¡lisis de Deals", "7 min", "CÃ³mo Analizar un Deal Inmobiliario MÃ¡s RÃ¡pido"], ["Financiamiento", "8 min", "DSCR vs BRRR: Lo Que Deben Saber los Inversionistas"], ["AnÃ¡lisis de Deals", "12 min", "GuÃ­a Completa de AnÃ¡lisis de Deals Inmobiliarios"], ["ConstrucciÃ³n", "9 min", "CÃ³mo Estimar Costos de ConstrucciÃ³n Desde Planos"], ["Financiamiento", "14 min", "Estrategias DSCR y BRRR para Inversionistas"], ["Takeoffs", "6 min", "Errores Comunes de Takeoff que DaÃ±an tu Ganancia"], ["ConstrucciÃ³n", "11 min", "Presupuestos y Estimaciones de ConstrucciÃ³n"], ["GestiÃ³n de Proyecto", "8 min", "QuÃ© Rastrear Durante una ConstrucciÃ³n Desde Cero"], ["Takeoffs", "10 min", "Takeoffs de ConstrucciÃ³n y EstimaciÃ³n con Planos"], ["Cierre", "7 min", "CÃ³mo las Punch Lists Reducen Retrasos"], ["GestiÃ³n de Proyecto", "13 min", "GestiÃ³n de ConstrucciÃ³n Desde Cero"], ["Cierre", "9 min", "Punch Lists y Cierre de ConstrucciÃ³n"]]
    : learningArticles;
  const [active, setActive] = useState(null);
  if (active) return <div className="space-y-6"><button onClick={() => setActive(null)} className="secondary-button">â† {t.back}</button><GlassPanel><p className="text-sm font-black text-amber-300">{active[0]} Â· {active[1]}</p><h2 className="mt-3 text-4xl font-black text-white">{active[2]}</h2><div className="mt-6 grid gap-4 md:grid-cols-3"><Info title={language === "es" ? "FÃ³rmulas" : "Formulas"} text="ARV, ROI, cap rate, cash-on-cash, DSCR, 70% rule, max offer, mortgage payment, and rent cash flow." /><Info title={language === "es" ? "Proceso" : "Process"} text="Start with fast screening, verify comps, stress-test costs, document assumptions, and export a clean investor packet." /><Info title={language === "es" ? "Checklist" : "Checklist"} text="Comps, scope, permits, financing, insurance, taxes, contractor bids, timeline, exit strategy, and risk buffer." /></div><p className="mt-6 max-w-4xl text-lg leading-8 text-slate-300">{language === "es" ? "Esta guÃ­a convierte el tema en pasos prÃ¡cticos para operadores. Usa los cÃ¡lculos de Operitron para revisar nÃºmeros, guardar supuestos, comparar escenarios y preparar decisiones con menos ruido." : "This guide turns the topic into practical operator steps. Use Operitron calculations to review numbers, save assumptions, compare scenarios, and prepare cleaner decisions with less noise."}</p><button onClick={() => go("projectTools")} className="primary-button mt-6">{language === "es" ? "Abrir herramientas" : "Open tools"}</button></GlassPanel></div>;
  return <div className="space-y-7"><GlassPanel><p className="text-sm font-black uppercase tracking-widest text-amber-300">{t.learning}</p><h2 className="mt-2 text-4xl font-black text-white">{language === "es" ? "GuÃ­as para Inversionistas y Constructores" : "Guides for Real Estate Investors & Builders"}</h2><p className="mt-3 max-w-3xl text-slate-400">{language === "es" ? "Estrategias prÃ¡cticas, marcos de trabajo y herramientas para analizar deals, gestionar construcciÃ³n y operar proyectos rentables." : "Practical strategies, frameworks, and tools for analyzing deals, managing construction, and operating profitable projects."}</p><button onClick={() => go("pricing")} className="primary-button mt-6">{t.startTrial}</button></GlassPanel><div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{articles.map((article) => <button key={article[2]} onClick={() => setActive(article)} className="glow-card rounded-[2rem] border border-white/10 bg-white/[.055] p-6 text-left shadow-2xl shadow-black/20 backdrop-blur-xl hover:border-amber-400/40"><p className="text-sm font-black text-amber-300">{article[0]} Â· {article[1]}</p><h3 className="mt-3 text-xl font-black text-white">{article[2]}</h3><p className="mt-3 text-sm leading-6 text-slate-400">{language === "es" ? "Una guÃ­a prÃ¡ctica para operadores con ejemplos, fÃ³rmulas y puntos de control del proyecto." : "A practical operator-focused guide with examples, formulas, and project checkpoints."}</p><span className="mt-5 inline-flex font-black text-amber-300">{t.readGuide} â†’</span></button>)}</div></div>;
}

function KnowledgeBase({ t, language }) {
  const faqs = language === "es"
    ? [
      ["Â¿CÃ³mo automatiza la IA los cÃ¡lculos de materiales a partir de planos?", "La IA puede leer planos subidos, aplicar una escala, convertir mediciones a dimensiones reales y organizar cantidades por categorÃ­a con factores de desperdicio configurables."],
      ["Â¿CuÃ¡l es el beneficio del rastreador de construcciÃ³n por fases?", "Un rastreador por fases ayuda a visualizar dependencias crÃ­ticas, evitar cuadrillas inactivas y reducir costos de financiamiento causados por retrasos."],
      ["Â¿CÃ³mo maneja el Analizador de Deals la estrategia BRRR?", "Modela compra, rehabilitaciÃ³n, renta, refinanciamiento, extracciÃ³n de capital, DSCR y flujo de caja posterior al refinanciamiento."],
      ["Â¿CÃ³mo mejora la herramienta de punch list el cierre de proyectos?", "Organiza deficiencias por oficio, prioridad, responsable y estado para cerrar pendientes con menos fricciÃ³n."],
    ]
    : [
  ...
  return <GlassPanel><h2 className="text-4xl font-black text-white">{t.knowledge}</h2><p className="mt-3 text-slate-400">{language === "es" ? `Respuestas detalladas sobre cÃ³mo ${t.brand} potencia cada fase de la inversiÃ³n inmobiliaria.` : `Detailed answers about how ${t.brand} supports each phase of real estate investing.`}</p><div className="mt-8 space-y-4">{faqs.map(([q, a]) => <details key={q} className="rounded-3xl border border-white/10 bg-slate-950/70 p-5"><summary className="cursor-pointer text-lg font-black text-white">{q}</summary><p className="mt-4 leading-7 text-slate-300">{a}</p></details>)}</div></GlassPanel>;
}
...
function AdminCheckoutDiagnostics({ diagnostics, language }) {
  ...
  const rows = [
    ...
    [isEs ? "CÃ³digo de respuesta API" : "API response code", diagnostics.apiResponseCode],
    [isEs ? "Mensaje de error" : "Error message", diagnostics.errorMessage],
  ];
  if (diagnostics.missingConfiguration?.length) rows.push([isEs ? "ConfiguraciÃ³n faltante" : "Missing configuration", diagnostics.missingConfiguration.join(", ")]);
  ...
  return <div className="rounded-2xl border border-amber-300/30 bg-amber-300/[.08] p-4 text-left shadow-[0_0_30px_rgba(251,191,36,.08)]">
    <p className="text-sm font-black uppercase tracking-widest text-amber-200">{isEs ? "DiagnÃ³stico administrativo de checkout" : "Admin checkout diagnostics"}</p>
...
function PricingPlans({ language, user, go }) {
  ...
  const monthlyFeatures = isEs
    ? ["Proyectos y deals ilimitados", "Analizador de deals y fix-and-flip", "Calculadoras DSCR, BRRR y cash-out", "Rastreador de construcciÃ³n", "Takeoff de materiales con IA", "Punch list", "GestiÃ³n de subcontratistas y ofertas", "Compartir proyectos y colaborar", "Asistente IA en cada herramienta"]
  ...
  const plans = [
    { id: "monthly", name: isEs ? "Mensual" : "Monthly", price: "$29.99", cadence: isEs ? "/mes" : "/month", note: isEs ? "Prueba gratis de 3 dÃ­as" : "3-day free trial", detail: isEs ? "Acceso flexible mes a mes." : "Flexible month-to-month access.", features: monthlyFeatures },
    { id: "annual", name: isEs ? "Anual" : "Annual", price: "$249.99", cadence: isEs ? "/aÃ±o" : "/year", note: isEs ? "Ahorra mÃ¡s de 30% Â· Prueba gratis de 3 dÃ­as" : "Save over 30% Â· 3-day free trial", detail: isEs ? "El mejor valor para operadores activos." : "Best value for active operators.", features: annualFeatures, featured: true },
  ];
...
function PasswordField({ value, onChange, placeholder, autoComplete, visible, onToggle, language }) {
  ...
  return <div>
    <div className="relative">
      ...
      />
      <button type="button" onClick={onToggle} aria-label={visible ? (language === "es" ? "Ocultar contraseÃ±a" : "Hide password") : (language === "es" ? "Mostrar contraseÃ±a" : "Show password")} className="absolute right-2 top-1/2 grid h-11 w-11 -translate-y-1/2 place-items-center rounded-xl text-slate-400 transition hover:bg-white/5 hover:text-cyan-200">{visible ? <EyeOff size={20} /> : <Eye size={20} />}</button>
    </div>
    <div className="mt-2 flex flex-wrap items-center gap-2 text-xs font-bold">
      <span className="text-slate-500">{language === "es" ? "Las contraseÃ±as distinguen mayÃºsculas y minÃºsculas." : "Passwords are case-sensitive."}</span>
      {capsLock && <span className="rounded-full border border-amber-300/30 bg-amber-300/10 px-3 py-1 text-amber-200">{language === "es" ? "Bloq MayÃºs activado" : "Caps Lock is on"}</span>}
    </div>
  </div>;
}
...
function SettingsPage({ t, language, user, setUser, go, back, signOut, passwordRecovery, setPasswordRecovery }) {
  ...
  const ui = isEs
    ? { register: "Registrarse", login: "Iniciar sesiÃ³n", details: "Crea tu cuenta de Operitron. Las herramientas se activan al iniciar una suscripciÃ³n o prueba mediante Stripe.", recoveryDetails: "Crea una nueva contraseÃ±a segura para tu cuenta.", fullName: "Nombre completo", company: "Empresa", phone: "TelÃ©fono", dashboard: "Volver al panel", signOut: "Cerrar sesiÃ³n", back: "Volver", save: "Crear cuenta", secure: "Tu cuenta se protege mediante autenticaciÃ³n segura y verificaciÃ³n por correo." }
  ...
  async function resetPassword() {
    if (!supabase) return setStatus(t.authUnavailable);
    if (!email.trim()) return setStatus(isEs ? "Ingresa tu correo electrÃ³nico primero." : "Enter your email address first.");
    setSubmitting(true);
  ...
  return (
    <GlassPanel>
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <button onClick={back} className="secondary-button">â† {ui.back}</button>
      </div>
      <div className="mt-7 grid gap-6 lg:grid-cols-[minmax(280px,36rem)_1fr]">
        <form onSubmit={submitAuth} className="grid gap-3">
          ...
          <p className="pt-2 text-sm text-slate-400">{isEs ? "Â¿Necesitas ayuda?" : "Need help?"} <a className="font-bold text-cyan-300 hover:text-cyan-200" href="mailto:support@operitron.com">support@operitron.com</a></p>
        </form>
        <div className="hidden rounded-3xl border border-white/10 bg-gradient-to-br from-cyan-400/10 via-slate-950 to-purple-500/10 p-7 lg:block">
          ...
          <ul className="mt-6 space-y-3 text-sm font-bold text-slate-300">
            ...
            <li className="flex gap-2"><CheckCircle2 className="text-cyan-300" size={18} />{isEs ? "FacturaciÃ³n segura mediante Stripe" : "Secure billing through Stripe"}</li>
            <li className="flex gap-2"><CheckCircle2 className="text-cyan-300" size={18} />{isEs ? "RecuperaciÃ³n de contraseÃ±a por correo" : "Email password recovery"}</li>
          </ul>
        </div>
      </div>
    </GlassPanel>
...
function LegalPage({ type, language }) {
  ...
  const spanish = {
    privacy: { title: "PolÃ­tica de Privacidad", intro: "Esta PolÃ­tica de Privacidad explica cÃ³mo OPERITRON.COM recopila, usa y protege informaciÃ³n al usar nuestro software inmobiliario y de construcciÃ³n.", sections: [["InformaciÃ³n que Recopilamos", "Podemos recopilar datos de cuenta, correo, empresa, telÃ©fono, actividad de autenticaciÃ³n, proyectos guardados, cÃ¡lculos, documentos, colaboradores y comunicaciones de soporte."], ["Uso de la InformaciÃ³n", "Usamos la informaciÃ³n para autenticar usuarios, operar espacios de trabajo, guardar anÃ¡lisis, habilitar facturaciÃ³n, brindar soporte y mantener la seguridad del servicio."], ["Pagos y Proveedores", "Stripe procesa los pagos. Supabase puede administrar autenticaciÃ³n y datos guardados. Los proveedores autorizados de datos procesan bÃºsquedas solicitadas por usted bajo sus propias polÃ­ticas."], ["Seguridad y RetenciÃ³n", "Usamos controles de acceso y polÃ­ticas a nivel de fila para limitar el acceso de cada usuario a sus propios registros. NingÃºn servicio en lÃ­nea garantiza seguridad absoluta."], ["Sus Opciones", "Puede solicitar acceso, correcciÃ³n o eliminaciÃ³n de informaciÃ³n de su cuenta comunicÃ¡ndose con soporte de Operitron."]]},
    terms: { title: "TÃ©rminos de Servicio", intro: "Estos TÃ©rminos regulan el uso de OPERITRON.COM, una plataforma para inversionistas y operadores de construcciÃ³n.", sections: [["Responsabilidad de Cuenta", "Debe proporcionar informaciÃ³n precisa, proteger sus credenciales y responsabilizarse por la actividad de su cuenta y colaboradores invitados."], ["Uso Permitido", "Puede usar Operitron para organizar proyectos, analizar inversiones, estimar construcciÃ³n y preparar reportes. No puede interrumpir el servicio ni intentar acceso no autorizado."], ["Suscripciones y Pruebas", "El acceso pagado se ofrece mediante planes mensuales y anuales con prueba gratis de tres dÃ­as cuando se muestre en checkout. Stripe controla la facturaciÃ³n aceptada por usted."], ["Datos del Cliente", "Usted conserva propiedad de su contenido y autoriza su procesamiento limitado para prestar el servicio."], ["LimitaciÃ³n de Responsabilidad", "En la medida permitida por ley, Operitron no responde por pÃ©rdidas de inversiÃ³n, sobrecostos de construcciÃ³n o resultados de financiamiento basados en resultados de la plataforma."]]},
    refund: { title: "PolÃ­tica de Reembolsos", intro: "Buscamos que la facturaciÃ³n de Operitron sea clara y predecible.", sections: [["Prueba Gratis de Tres DÃ­as", "Las nuevas suscripciones elegibles pueden incluir una prueba de tres dÃ­as segÃºn se muestre en checkout. Cancele antes del fin de la prueba para evitar el primer cargo."], ["Cancelaciones", "Puede cancelar en el portal de facturaciÃ³n. La cancelaciÃ³n evita renovaciones futuras y normalmente mantiene acceso hasta terminar el perÃ­odo pagado actual."], ["Solicitudes de Reembolso", "Los cargos de suscripciÃ³n generalmente no son reembolsables una vez iniciado el perÃ­odo, salvo exigencia legal o error de facturaciÃ³n."], ["Promociones", "Los descuentos se aplican mediante Stripe Checkout y no se cambian por efectivo ni se aplican retroactivamente."]]},
    disclaimer: { title: "Aviso Legal", intro: "Operitron es software de apoyo para decisiones, no asesorÃ­a profesional ni garantÃ­a de resultados.", sections: [["Estimaciones Financieras", "ARV, ROI, cap rate, cash-on-cash, DSCR, pagos hipotecarios, ofertas mÃ¡ximas, flujos de efectivo y presupuestos dependen de datos y supuestos del usuario."], ["Datos PÃºblicos y de Propiedad", "Registros, comparables, impuestos y datos pÃºblicos pueden estar incompletos o atrasados. Confirme hechos importantes en fuentes autorizadas."], ["Resultados de ConstrucciÃ³n", "Takeoffs, cronogramas, punch lists, alcances, estimaciones y resÃºmenes asistidos por IA son ayudas de planeaciÃ³n. Consulte profesionales licenciados."], ["Sin RelaciÃ³n Profesional", "Operitron no proporciona asesorÃ­a legal, fiscal, de avalÃºo, corretaje, prÃ©stamo, arquitectura, ingenierÃ­a o construcciÃ³n."]]},
  };
  const data = (language === "es" ? spanish : english)[type];
  return <article className="mx-auto max-w-4xl rounded-[1.5rem] border border-white/10 bg-white/[.055] p-5 shadow-2xl shadow-black/20 sm:rounded-[2rem] sm:p-6 md:p-10"><p className="text-xs font-black uppercase tracking-[0.24em] text-cyan-300 sm:text-sm">OPERITRON.COM</p><h1 className="mt-3 text-3xl font-black leading-tight text-white sm:text-4xl md:text-5xl">{data.title}</h1><p className="mt-5 max-w-3xl text-base leading-7 text-slate-300 sm:text-lg sm:leading-8">{data.intro}</p><p className="mt-4 text-sm font-bold text-slate-500">{language === "es" ? "Vigente: 25 de mayo de 2026" : "Effective: May 25, 2026"}</p><div className="mt-8 space-y-7 sm:mt-10 sm:space-y-8">{data.sections.map(([heading, text]) => <section key={heading} className="border-t border-white/10 pt-6 sm:pt-7"><h2 className="text-xl font-black text-white">{heading}</h2><p className="mt-3 whitespace-normal break-words text-base leading-7 text-slate-300 sm:leading-8">{text}</p></section>)}</div><p className="mt-8 rounded-2xl border border-cyan-300/15 bg-cyan-300/5 p-4 text-sm leading-7 text-slate-400 sm:mt-10">{language === "es" ? "Contacto: Para preguntas sobre estas polÃ­ticas, escriba a " : "Contact: For questions about these policies, email "}<a href="mailto:support@operitron.com" className="font-bold text-cyan-300 hover:text-cyan-200">support@operitron.com</a>.</p></article>;
}
