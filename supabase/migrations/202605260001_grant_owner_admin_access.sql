...
begin
  ...
  values (
    ...
    case when lower(coalesce(new.email, '')) = 'your-admin-email@example.com' then 'admin' else 'user' end
  )
  on conflict (id) do update
  set email = excluded.email,
      role = case
        when lower(excluded.email) = 'your-admin-email@example.com' then 'admin'
        else profiles.role
      end,
      updated_at = now();
  return new;
